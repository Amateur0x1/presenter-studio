...
| 翻页 | ← → 方向键，或触屏滑动 |
| 开始录制 | 点击顶部工具栏「● 开始」 |
| 摄像头预览 | 页面打开后自动开启；失败时点击右上角「重试摄像头」 |
| 暂停/继续 | 按 `Space` 键 |
| 停止录制 | 按 `Esc` 键 |
| 编辑文本 | 双击文本块 |
| 移动文本 | 拖拽文本块 |
| 退出编辑 | 按 `Esc` 或点击空白处 |
| 保存 HTML | 点击「保存 HTML」，选择要覆盖的 `.html` 文件 |

> 未点击「保存 HTML」前，浏览器里的文字和位置修改只是临时状态，刷新页面会丢失。

## 录制流程

1. 打开页面后授权摄像头，右上角预览用于调整构图
2. 点击「开始」→ 浏览器请求麦克风和标签页捕获权限
3. 授权后工具栏和预览自动隐藏，开始录制
4. 正常翻页讲解，所有操作都会被录入 slides 视频
5. 按 `Space` 可暂停，再按继续
6. 按 `Esc` 停止 → 弹出文件夹选择器 → 两个视频文件保存到该文件夹
